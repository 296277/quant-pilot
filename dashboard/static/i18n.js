(() => {
  ...
  const entries = [
    ...
    ['原始候选 vs 用户版本', 'Original Candidate vs User Version'],
    ['当前参数版本与生成时参数的研究结果对比', 'Compare the current parameter version with the generated candidate'],
    ['原始候选', 'Original Candidate'], ['用户版本', 'User Version'], ['变化', 'Change'],
    ['仅模拟环境', 'Simulation Only'], ['模拟环境 · 不会实盘', 'Simulation Only · No Live Orders'],
    ['重新开始回放', 'Restart Replay'], ['导出 CSV', 'Export CSV'], ['清空账户', 'Clear Account'],
    ['已实现盈亏', 'Realized P&L'], ['已卖出部分', 'Closed Positions'], ['手续费', 'Fees'],
    ['笔待成交', 'pending orders'], ['手动模拟订单', 'Manual Paper Order'],
    ['当前回放日期 · A 股遵守 T+1/100股整手，虚拟货币支持小数仓位', 'Current replay date · A-shares enforce T+1 and 100-share lots; crypto supports fractional positions'],
    ['订单类型', 'Order Type'], ['市价', 'Market'], ['限价（待成交可撤）', 'Limit (cancel while pending)'],
    ['价格（限价单）', 'Price (Limit Orders)'], ['提交模拟订单', 'Submit Paper Order'],
    ['条记录，含信号来源和拒单原因', 'records with signal sources and rejection reasons'],
    ['信号来源', 'Signal Source'], ['原因', 'Reason'], ['操作', 'Action'], ['撤单', 'Cancel'],
    ['已成交', 'Filled'], ['部分成交', 'Partially Filled'], ['待成交', 'Pending'], ['已撤单', 'Cancelled'],
    ['已拒单', 'Rejected'], ['受限', 'Blocked'],
    ['确定撤销这笔待成交模拟订单吗？', 'Cancel this pending paper order?'],
    ['确定从留出样本起点重新开始回放吗？当前订单和盈亏记录将清空。', 'Restart from the holdout sample? Current orders and P&L history will be cleared.'],
    ['模拟订单已处理', 'Paper order processed'], ['模拟订单已撤销', 'Paper order cancelled'],
    ['已重新开始模拟回放', 'Paper replay restarted'],
    ['OKX Demo · 模拟环境', 'OKX Demo · Simulation Environment'],
    ['所有订单强制发送模拟交易标记，不会进入实盘账户。', 'Every order is forced into simulated trading and never reaches a live account.'],
    ['涨停约束', 'Limit-up Threshold'], ['跌停约束', 'Limit-down Threshold'],
    ['涨跌停阈值可配置', 'Configurable price-limit thresholds'], ['加密资产：最小 0.000001', 'Crypto minimum: 0.000001'],
