...
function terminalToolbar(title, note, refreshAction = '', dataStatus = null) {
  return `<div class="terminal-page-head"><div><h2>${escapeHtml(title)}</h2><span>${escapeHtml(note || '')}</span>${dataStatus ? dataStateBadge(dataStatus.status, dataStatus.label) : ''}</div>${refreshAction ? `<button class="secondary-button" data-terminal-action="${refreshAction}" title="刷新">↻ 刷新</button>` : ''}</div>`;
}
...
function renderMarket() {
  ...
  host.innerHTML = `${terminalToolbar('市场看板', `${data.source} · ${data.as_of} · ${scope}`, 'refresh-market', {status:data.stale?'cached':'live',label:data.stale?'缓存快照':'公开市场快照'})}
...
function renderConcepts(){const host=$('#conceptContent'),d=terminalState.concepts;terminalHostReady(host);if(!terminalState.selectedConcept)terminalState.selectedConcept=d.groups[0]?.id;const selected=d.groups.find(g=>g.id===terminalState.selectedConcept)||d.groups[0];host.innerHTML=`${terminalToolbar('概念分析',`${d.source} · ${d.as_of} · ${d.scope_note}`,'refresh-concepts',{status:d.stale?'cached':'live',label:d.stale?'缓存快照':'项目观察池'})}${d.warning?`<div class="terminal-warning">${escapeHtml(d.warning)}</div>`:''}<div class="concept-kpis">${[['最强主线',d.strongest?.name,tPct(d.strongest?.change)],['最大风险',d.weakest?.name,tPct(d.weakest?.change)],['覆盖板块',d.groups.length,'项目配置池'],['资金活跃',d.groups.slice().sort((a,b)=>b.amount-a.amount)[0]?.name,tMoney(d.groups.slice().sort((a,b)=>b.amount-a.amount)[0]?.amount)]].map(([l,v,n])=>`<div><span>${l}</span><strong>${escapeHtml(v||'—')}</strong><small>${escapeHtml(n||'')}</small></div>`).join('')}</div><div class="heat-columns terminal-panel"><div><h4 class="positive">领涨方向</h4>${d.leaders.map(groupRow).join('')}</div><div><h4 class="negative">领跌方向</h4>${d.laggards.map(groupRow).join('')}</div></div>${selected?conceptDetail(selected):''}`;bindTerminalActions(host);}
