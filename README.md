...
> **当前状态：早期实验版本。** 项目整体仍比较粗糙，部分数据源、分析页面、策略参数、
> 异常处理和交易适配功能尚不完善，稳定性、准确性与使用体验仍需持续改进。
...
| 本地模拟 | 将候选策略带入历史留出样本，逐日或快速回放现金、持仓、订单、盈亏和净值 |
| OKX Demo | 加密保存 Demo 凭据、同步资金/持仓/委托/成交、调整策略参数、计算最新信号并二次确认模拟下单 |
| miniQMT | 填写本机 QMT 目录与账号，只读同步 A 股账户信息，不调用委托接口 |
| 数据与环境 | 本地 CSV 数据目录、网络快照回退、Python/NumPy/Pandas/策略引擎和腾讯行情自检 |
...
4. 选择候选后查看信号规则、参数、净值曲线和最近交易。
5. 将策略部署到本地模拟账户进行历史回放；虚拟货币策略也可带到 OKX Demo。
6. 在 OKX Demo 中修改参数、计算最后一根已完成日线信号，并在人工检查后确认模拟订单。
